# 1) Write a fresh generator (no escapes) to C:\Windows\Temp\trafficgen.ps1
$script = @'
param(
...
try {
  ...
  function Ensure-Package($name){
    ...
    } else { Log "Package $name already present." }
  }
  ...
  # Ensure client tooling (not servers) for traffic generation
  Ensure-Choco
  Ensure-Package 'postgresql'           # psql.exe client
  ...
  while($true){
    try{
      ...
      # --- 2) HTTP 1–10MB → FTP /uploads → delete remote & local
...
const col = dbb.getCollection("__COL__");
let bulk = [];
...
col.drop();
'@
      $mongoJs = $mongoJs -replace '__COL__',$col
      Log 'mongosh ops'
...
'@
Set-Content -Path 'C:\Windows\Temp\trafficgen.ps1' -Value $script -Encoding UTF8
Write-Host "Wrote C:\Windows\Temp\trafficgen.ps1"
